...
- **说说广场** (`/moments`): 所有说说列表 🆕
- **项目列表** (`/projects`): 项目展示页面 🆕
- **项目详情** (`/projects/:id`): 项目详细介绍 🆕
- **留言板** (`/guestbook`): 留言墙页面 🆕
- **关于页面** (`/about`): 个人介绍
- **登录页面** (`/login`): 管理员登录
...
- **项目管理** (`/admin/projects`): 项目 CRUD、批量操作 🆕
- **项目编辑** (`/admin/projects/create`, `/admin/projects/:id/edit`) 🆕
- **留言管理** (`/admin/messages`): 留言审核、删除、批量操作 🆕
...
- ✅ **游客留言** - 无需登录，填写昵称和邮箱即可留言
- ✅ **审核机制** - 游客留言需要审核，登录用户自动通过
- ✅ **心情选择** - 5 种心情（开心/伤心/生气/兴奋/思考）
- ✅ **回复功能** - 支持对留言进行回复
- ✅ **点赞功能** - 为留言点赞
- ✅ **卡片布局** - 瀑布流网格布局，随机背景颜色
- ✅ **心情筛选** - 按心情类型筛选留言
...
- ✅ **项目筛选** - 按类型/状态/技术栈筛选
- ✅ **搜索功能** - 关键词搜索（防抖处理）
- ✅ **项目详情** - Hero 头部、Markdown 内容、截图画廊、视频嵌入
- ✅ **项目信息** - 技术栈、状态、团队规模、时间线
- ✅ **外部链接** - 演示/GitHub/文档/视频链接
- ✅ **后台管理** - 完整 CRUD、批量操作、精选标记
- ✅ **项目编辑** - Markdown 编辑器、封面/截图上传
- ✅ **浏览统计** - 自动记录浏览次数
- ✅ **响应式设计** - 移动端适配
